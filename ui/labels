...
var labelWidget = function(fc, prop, options) {
  ...
  if (helpers.array.contains(fonts, this.options.fontType)) {  
    this.fontType = this.options.fontType
  } else {
    this.fontType = def.fontType
  }
  
  if (helpers.array.contains(sizes[this.fontType], this.options.fontSize.toString())) {
  ...
  this.panel = ui.Panel({
    widgets: [fontPanel, scalePanel, colors],
...
labelWidget.prototype.getLabelsLayer = function() {
